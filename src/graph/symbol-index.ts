/**
 * Browser-safe symbol index — builds lookup structures from graph data.
 *
 * Pure computation with no Node.js or server-only dependencies.
 * Used by both the viewer (client-side) and the server.
 */

import { getDocPath, renderNodeMarkdown } from './graph-renderer.js';
 ...
 *
 * Browser-safe alternative to `path.dirname` from `node:path`.
 *
 * @param filePath - A forward-slash-separated file path
 */
function dirnameBrowser(filePath: string): string {
...
/**
 * Build the docs index grouped by source directory.
 *
...
export function buildIndexResponse(index: SymbolIndex) {
  ...
  for (const [, entry] of index.entries) {
    const dir = dirnameBrowser(entry.sourcePath) || '.';
...
/**
 * Build the doc response for a single symbol.
 *
 * Generates markdown on-the-fly from graph data and enriches the response
 * with metadata from the symbol index. Returns the raw mermaid source for
 * the dependency graph (caller can render to SVG server-side if desired).
 *
